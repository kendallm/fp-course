...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s b
  (<$>) fab st =
    State (\s -> (fab (eval st s), exec st s))
...
findM ::
  ...
  -> List a
  -> f (Optional a)
-- Can this be done with foldRight??
...
firstRepeat as =
  eval
    (findM (\a -> State (\seen -> if S.member a seen then (True, seen) else (False, S.insert a seen) )) as) 
    S.empty
...
isHappy x =
  let square = join (*)
      digits a = square . digitToInt <$> show' a
      r = firstRepeat (produce (toInteger . sum . digits) x)
  in contains 1 r
